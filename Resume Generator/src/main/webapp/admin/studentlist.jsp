...
<body>
	...
	<div class="container-fluid">
		<div class="overview">
			<div class="title">
				<h2 class="m-4">
					<i class="uil uil-graduation-cap"></i>All Students
				</h2>
				<div>
					<form class="m-4" action="../SearchStudent" method="post"
						class="search-bar-container">
						<input type="search" id="search-bar" name="studentName"
		...
		<table class="table table-responsive-sm text-center">
			...
			<tbody class="rows">
				...
				<%
				List<StudentPersonel> list1 = (ArrayList<StudentPersonel>)session.getAttribute("list");
				int i =0;
				for (StudentPersonel s : list1) {
					i++;
				%>

				<tr>
					<td><%=i %></td>
					<td><%=s.getScholarNo() %></td>
					<td><%=s.getStName() %></td>
					<td><%=s.getStEnroll() %></td>
					<td><%=s.getStDept() %></td>
					<td><a href="../UpdateStudentInfo?sid=<%=s.getScholarNo() %>" class="btn btn-sm edit"><i
							class="fas fa-edit"></i>Edit</a> <a href="../DeleteStudent?id=<%=s.getScholarNo()%>"
