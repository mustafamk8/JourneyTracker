<%@page import="com.resume.bean.Faculty"%>
<%@page import="com.resume.bean.StudentPersonel"%>
<%@page import="java.util.List"%>
<%@page import="com.resume.model.AdminModel"%>
<%@include file="../admin/components/adminLogout.jsp" %>
<%@page import="com.resume.bean.Admin"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="stylesheet" href="../admin/css/admindashboard.css">
<link rel="stylesheet"
	href="https://unicons.iconscout.com/release/v4.0.0/css/line.css">
<link rel="stylesheet"
	href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
<link rel="stylesheet"
	href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">

<title>Admin Dashboard Panel</title>
</head>
<body>
<%
Admin a  = (Admin)session.getAttribute("current-user");
AdminModel model = new AdminModel();
List<StudentPersonel> studentList = model.getAllStudents();
List<Faculty> facultyList = model.getAllFaculty();
List<Integer> noList = model.getAllStudentsDept();
%>
	<nav>
		<a href="#dashboard">
			<div class="logo-name">
				<span class="logo-icon">JT</span> <span class="logo_name">Journey
					Tracker</span>
			</div>
		</a>

		<div class="menu-items">
			<ul class="nav-links">
				<li><a href="#dashboard"> <i class="uil uil-estate"></i> <span
						class="link-name">Dashboard</span>
				</a></li>
				<li><a href="#studentsAndFaculty"> </i><i
						class="far fa-user fa-3x"></i> <span class="link-name">Students
							and Faculty</span>
				</a></li>
				<li><a href="#upload"> <i class="uil uil-files-landscapes"></i>
						<span class="link-name">Upload File</span>
				</a></li>
			</ul>

			<ul class="logout-mode">
				<li><a href="../AdminLogout"> <i class="uil uil-signout"></i> <span
						class="link-name">Logout</span>
				</a></li>
				</li>
			</ul>
		</div>
	</nav>

	<section class="dashboard" id="dashboard">
		<div class="top">
			<i class="uil uil-bars sidebar-toggle"></i>

			<div class="profile">
				<div class="profile-name"><%=a.getName() %></div>
				<div class="profile-dropdown">
					<button type="button" class="dropdown-toggle">
						<i class="uil uil-user" style="font-size: 18px;"></i>
					</button>
					<ul class="dropdown-menu">
						<li><a href="#">Update Password</a></li>
						<li><a href="../AdminLogout">Logout</a></li>
					</ul>
				</div>
			</div>
		</div>

		<div class="dash-content">
			<div class="overview">
				<div class="title">
					<i class="uil uil-tachometer-fast-alt"></i> <span class="text">Dashboard</span>
				</div>
				<%@include file="../components/message.jsp"%>
				<div class="boxes">
					<div class="box">
						<span class="text">Total Students</span> <span class="number"><%=studentList.size() %></span>
					</div>
					<div class="box" style="background-color: #c2a3df">
						<span class="text">Total Faculty</span> <span class="number"><%=facultyList.size() %></span>
					</div>
				</div>

				<div style="margin-top: 20px;">
					<h4>Total Students Branch-wise:</h4>
				</div>

				<div class="boxes2">
					<div class="box">
						<span class="text">CSE </span> <span class="number"><%=noList.get(0) %></span>
					</div>
					<div class="box" style="background-color: #c2a3df">
						<span class="text">IT</span> <span class="number"><%=noList.get(1) %></span>
					</div>
					<div class="box" style="background-color: #E7D1FC">
						<span class="text">CSIT</span> <span class="number"><%=noList.get(2) %></span>
					</div>
				</div>
				<div class="boxes3">
					<div class="box">
						<span class="text">CSE-DS</span> <span class="number"><%=noList.get(3) %></span>
					</div>
					<div class="box" style="background-color: #c2a3df">
						<span class="text">CSE-AIML</span> <span class="number"><%=noList.get(4) %></span>
					</div>
					<div class="box" style="background-color: #E7D1FC">
						<span class="text">CSE-IOT</span> <span class="number"><%=noList.get(5) %></span>
					</div>

				</div>
			</div>
		</div>
	</section>

	<section class="dashboard" id="studentsAndFaculty">
		<div class="dash-content">
			<div class="overview">
				<div class="title">
					<i class="far fa-user fa-3x"></i> <span class="text">All
						Students and Faculty</span>
				</div>

				<div class="container" style="margin-top: 50px;">
					<div class="row"
						style="display: flex; justify-content: space-around; flex-wrap: wrap;">
						<div class="card-sl">
							<div class="card-image">
								<img
									src="data:image/jpeg;base64," />
							</div>
							<div class="card-heading">STUDENTS</div>
							<a href="allStudents.jsp" class="card-button">View All</a>
						</div>

						<div class="card-sl">
							<div class="card-image">
								<img
									src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQiMXywxgDeztlYhoDmX3-pFxI9dxxNO10t1u0iz1iH2mofQbzmoJU9jS5fh96vTuQbX2U&usqp=CAU" />
							</div>
							<div class="card-heading">Faculty</div>
							<a href="allFaculty.jsp" class="card-button">View All</a>
						</div>
					</div>
				</div>

			</div>

		</div>
	</section>

	<section class="dashboard" id="upload">
		<div class="dash-content">
			<div class="overview">
				<div class="title">
					<i class="uil uil-files-landscapes"></i> <span class="text">Upload
						file here</span>
				</div>
				<div style="display: flex; justify-content: space-around; margin-bottom:200px;">
					<form method="post" action="../UploadStudentServlet" enctype="multipart/form-data">
						<div>
							<div class="input-group">
								<label for="student">For Students :</label> <input type="file"
									name="studentName" class="form-control" id="inputGroupFile02"
									aria-describedby="inputGroupFileAddon04" aria-label="Upload"
									accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
							</div>
						</div>
						<p>
							<input type="submit" value="Upload">
						</p>
					</form>

					<form method="post" action="../UploadTeacherFile" enctype="multipart/form-data">
						<div>
							<div class="input-group">
								<label for="faculty">For Faculty :</label> <input type="file"
									class="form-control" id="inputGroupFile02"  name="excelFile"
									aria-describedby="inputGroupFileAddon04" aria-label="Upload"
									accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
							</div>
						</div>
						<p>
							<input type="submit" value="Upload">
						</p>
					</form>
				</div>

			</div>
		</div>
	</section>

	<script>
        const body = document.querySelector("body"),
        sidebar = body.querySelector("nav");
        sidebarToggle = body.querySelector(".sidebar-toggle");

        sidebarToggle.addEventListener("click", () => {
            sidebar.classList.toggle("close");
            if(sidebar.classList.contains("close")){
                localStorage.setItem("status", "close");
            }else{
                localStorage.setItem("status", "open");
            }
        })
    </script>

</body>
</html>