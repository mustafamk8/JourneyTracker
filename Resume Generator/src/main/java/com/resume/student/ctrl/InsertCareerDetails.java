...
@WebServlet("/InsertCareerDetails")
public class InsertCareerDetails extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int i = model.insertCareerDetails(c);
		
		HttpSession session = request.getSession();
		if(i!=0) {
			session.setAttribute("message", "Student Academic Career Details inserted Successfully");
			response.sendRedirect("career_details.jsp");
		}else {
